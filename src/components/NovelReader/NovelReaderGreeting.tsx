...
function NovelReaderGreeting({ novel }: NovelReaderGreetingProps) {
  return (
    <div className="w-full max-w-4xl relative overflow-y-auto h-screen scrollbar-hide pt-20 pb-8">
      {/* Фоновое изображение */}
      <div 
        ...
        {novel.homePage?.news && novel.homePage.news.length > 0 && (
          <div className="space-y-4 animate-fade-in">
            ...
            {novel.homePage.news.map((item) => (
              <div key={item.id} className="bg-card/90 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-border">
