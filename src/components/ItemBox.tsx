...
function ItemBox({ name, description, imageUrl, skipTyping, onComplete, textSpeed = 50, isTopMerged = false }: ItemBoxProps) {
  ...
  return (
    <Card className="bg-gradient-to-br from-card via-secondary/10 to-accent/20 backdrop-blur-sm border-0 shadow-2xl animate-scale-in max-w-md mx-auto rounded-xl md:rounded-2xl">
      <CardContent className={isTopMerged ? "p-2 md:p-4 lg:p-6 text-center" : "p-3 md:p-4 lg:p-8 text-center"}>
        <Badge variant="default" className={isTopMerged 
          ? "mb-1 md:mb-3 lg:mb-4 text-[10px] md:text-xs bg-primary text-primary-foreground border-0 shadow-lg px-1.5 md:px-3 py-0.5 md:py-1 rounded-full font-semibold"
        ...
        <div className={isTopMerged 
          ? "mb-2 md:mb-4 lg:mb-5 animate-scale-in flex justify-center p-1.5 md:p-3 lg:p-4 bg-background/50 rounded-xl md:rounded-2xl border-0 shadow-inner"
          ...
          {imageUrl && (imageUrl.startsWith('data:') || imageUrl.startsWith('http')) ? (
            <ZoomableImage
              ...
              className={isTopMerged 
                ? "max-w-[80px] md:max-w-[140px] lg:max-w-[200px] max-h-16 md:max-h-28 lg:max-h-36 object-contain rounded-lg md:rounded-xl drop-shadow-2xl"
            ...
            <div className={isTopMerged ? "text-3xl md:text-5xl lg:text-7xl drop-shadow-2xl" : "text-5xl md:text-6xl lg:text-9xl drop-shadow-2xl"}>{imageUrl}</div>
          ) : (
            <div className={isTopMerged 
              ? "w-16 h-16 md:w-24 md:h-24 bg-muted rounded-lg md:rounded-xl flex items-center justify-center text-muted-foreground"
              : "w-32 h-32 md:w-48 md:h-48 bg-muted rounded-xl md:rounded-2xl flex items-center justify-center text-muted-foreground"
            }>
              <div className={isTopMerged ? "text-2xl md:text-4xl" : "text-5xl md:text-6xl"}>📦</div>
            </div>
          )}
        </div>
        <h3 className={isTopMerged 
          ? "text-sm md:text-lg lg:text-xl font-bold text-primary mb-1 md:mb-2 lg:mb-3 drop-shadow-lg"
        ...
        <p className={`novel-text leading-snug md:leading-relaxed text-foreground font-medium ${
          isTopMerged ? "text-xs md:text-base lg:text-lg" : "text-sm md:text-base lg:text-lg"
        }`}>
          <TypewriterText 
            text={description}
            speed={textSpeed}
