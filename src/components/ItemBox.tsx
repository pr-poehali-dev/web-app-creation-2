...
function ItemBox({ name, description, imageUrl, skipTyping, onComplete, textSpeed = 50, isTopMerged = false }: ItemBoxProps) {
  ...
  return (
    <Card className="bg-gradient-to-br from-card via-secondary/10 to-accent/20 backdrop-blur-sm border-0 shadow-2xl animate-scale-in max-w-md mx-auto">
      <CardContent className={isTopMerged ? "p-4 md:p-6 text-center" : "p-4 md:p-8 text-center"}>
        <Badge variant="default" className={isTopMerged 
          ? "mb-3 md:mb-4 text-xs bg-primary text-primary-foreground border-0 shadow-lg px-2 md:px-3 py-0.5 md:py-1 rounded-full font-semibold"
        ...
        <div className={isTopMerged 
          ? "mb-4 md:mb-5 animate-scale-in flex justify-center p-2 md:p-4 bg-background/50 rounded-2xl border-0 shadow-inner"
          ...
          {imageUrl && (imageUrl.startsWith('data:') || imageUrl.startsWith('http')) ? (
            <ZoomableImage
              ...
              className={isTopMerged 
                ? "max-w-[140px] md:max-w-[200px] max-h-28 md:max-h-36 object-contain rounded-xl drop-shadow-2xl"
                : "max-w-[200px] md:max-w-xs max-h-48 md:max-h-64 object-contain rounded-2xl drop-shadow-2xl"
              }
            />
          ) : imageUrl ? (
            <div className={isTopMerged ? "text-5xl md:text-7xl drop-shadow-2xl" : "text-6xl md:text-9xl drop-shadow-2xl"}>{imageUrl}</div>
          ) : (
            <div className={isTopMerged 
        ...
        <h3 className={isTopMerged 
          ? "text-lg md:text-xl font-bold text-primary mb-2 md:mb-3 drop-shadow-lg"
        ...
        <p className={`novel-text leading-relaxed text-foreground font-medium ${
          isTopMerged ? "text-base md:text-lg" : "text-base md:text-lg"
        }`}>
          <TypewriterText 
            text={description}
            speed={textSpeed}
