...
function ItemBox({ name, description, imageUrl, skipTyping, onComplete, textSpeed = 50, isMerged = false }: ItemBoxProps) {
  ...
  return (
    <Card className="bg-gradient-to-br from-card via-secondary/10 to-accent/20 backdrop-blur-sm border-0 shadow-2xl animate-scale-in max-w-md mx-auto">
      <CardContent className={isMerged ? "p-3 md:p-4 text-center" : "p-4 md:p-8 text-center"}>
        <Badge variant="default" className={isMerged 
        ...
        <div className={isMerged 
          ...
          {imageUrl && (imageUrl.startsWith('data:') || imageUrl.startsWith('http')) ? (
            <ZoomableImage
              ...
              alt={name}
              className={isMerged 
                ? "max-w-[120px] md:max-w-[150px] max-h-24 md:max-h-32 object-contain rounded-xl drop-shadow-2xl"
                : "max-w-[200px] md:max-w-xs max-h-48 md:max-h-64 object-contain rounded-2xl drop-shadow-2xl"
              }
            />
          ) : imageUrl ? (
            <div className={isMerged ? "text-4xl md:text-6xl drop-shadow-2xl" : "text-6xl md:text-9xl drop-shadow-2xl"}>{imageUrl}</div>
          ) : (
            <div className={isMerged 
              ...
              <div className={isMerged ? "text-4xl" : "text-6xl"}>📦</div>
            </div>
          )}
        </div>
